...
// --- tsParticles Konfigurationen (NEU) ---
const particleConfigs = {
    clear: {}, // Leere Konfiguration zum Entfernen
    rain: {
        particles: {
            number: { value: 80, density: { enable: true, value_area: 800 } }, // Dichte angepasst
            color: { value: ["#93c5fd", "#60a5fa"] }, // Regenfarben (helleres Blau)
            shape: { type: "line" },
            opacity: { value: 0.5, random: true },
            size: { value: 10, random: { enable: true, minimumValue: 5 } }, // Längere Linien
            move: {
                enable: true,
                speed: 12, // Schneller
                direction: "bottom", // Rein nach unten
                straight: true,
                out_mode: "out", // Verschwinden unten
                bounce: false,
            },
        },
        interactivity: { detect_on: "canvas", events: { onclick: { enable: false }, onhover: { enable: false } } },
        detectRetina: true,
        backgroundMode: { enable: true, zIndex: 0 } // Wichtig, damit es hinter dem Inhalt ist
    },
    snow: {
        particles: {
            number: { value: 100, density: { enable: true, value_area: 800 } },
            color: { value: "#e5e7eb" }, // Schneefarbe
            shape: { type: "circle" },
            ...
            move: {
                ...
                speed: 1.5, // Langsam
                direction: "bottom",
                random: true, // Leichte Variation
                straight: false, // Nicht ganz gerade
                out_mode: "out",
                bounce: false,
    ...
    clouds: { // Sehr dezente, langsame Partikel als Nebel/Wolken
        particles: {
            number: { value: 20, density: { enable: true, value_area: 800 } },
            color: { value: "#ffffff" }, // Weiße Partikel
            shape: { type: "circle" },
            opacity: { value: 0.1, random: { enable: true, minimumValue: 0.05 } }, // Sehr transparent
            size: { value: 150, random: { enable: true, minimumValue: 80 } }, // Sehr groß
            move: {
                enable: true,
                speed: 0.5, // Sehr langsam
                direction: "top-right", // Leicht diagonal
                random: true,
                straight: false,
                out_mode: "out",
                bounce: false,
...
function setDynamicBackground(weathercode, isDay) {
    ...
    const isClear = wc <= 1;
    const isCloudy = (wc >= 2 && wc <= 3); // Nur Wolken
    const isFog = wc === 45 || wc === 48; // Nebel separat
    ...
    if (isDay === 0) { // Nacht
        ...
        else if (isCloudy) { colorClass = 'weather-cloudy-night'; particleConfig = particleConfigs.clouds; }
        else if (isFog) { colorClass = 'weather-cloudy-night'; particleConfig = particleConfigs.clouds; } // Nebel wie Wolken
        ...
        else colorClass = 'weather-cloudy-night'; // Default Nacht
    } else { // Tag
        if (isClear) colorClass = 'weather-clear-day';
        else if (isCloudy) { colorClass = 'weather-cloudy-day'; particleConfig = particleConfigs.clouds; }
        else if (isFog) { colorClass = 'weather-cloudy-day'; particleConfig = particleConfigs.clouds; } // Nebel wie Wolken
        else if (isRain) { colorClass = 'weather-rain'; particleConfig = particleConfigs.rain; }
        else if (isSnow) { colorClass = 'weather-snow'; particleConfig = particleConfigs.snow; }
        else colorClass = 'weather-cloudy-day'; // Default Tag
    }

    // Farb-Klasse am Body setzen
    ...
    // Kurze Verzögerung, damit der Farbwechsel zuerst sichtbar wird (optional)
    setTimeout(() => {
...
function showLoading(message = "Lade...") {
    ...
    // Keine Partikel während des Ladens
...
function showError(message) {
    ...
    // Hintergrund & Partikel zurücksetzen
